...
| Verification | Before version metadata changes, `uv run python scripts/release_gate.py --version 0.5.0` must reject only the expected version mismatch and the docs-currency selector must be proven to inspect public product documentation without hard-coding planning-artifact paths. Then `uv run python scripts/release_gate.py --version 0.5.0`, full lint/type/test suite, labeled Stage 1 smoke, `uv build`, `uvx twine check dist/*`, recorded hashes, macOS/Ubuntu clean-install smoke, human publication approval, `uv publish`, `gh release create`, and `uv run python scripts/verify_published_release.py --version 0.5.0` all succeed. |
