...
| Verification | Before metadata changes, `uv run python scripts/release_gate.py --version 0.5.0` must reject only the expected version mismatch and the docs-currency selector must be proven to inspect public product documentation without hard-coding planning-artifact paths. PR-1 is limited to that correction and its regression coverage, so its full quality gate remains green against the published `0.4.0` declarations. PR-2 changes canonical metadata, current public version declarations, changelog, and dependency manifest together; it then passes `uv run python scripts/release_gate.py --version 0.5.0`, full lint/type/test suite, labeled Stage 1 smoke, `uv build`, `uvx twine check dist/*`, recorded hashes, macOS/Ubuntu clean-install smoke, human publication approval, `uv publish`, `gh release create`, and `uv run python scripts/verify_published_release.py --version 0.5.0`. |
| Design reevaluation | Verify M24's exact merged command names, config migration, current release-tool flags, `check_docs_currency.py`, and required consumer smoke before branch creation. A release-tool mismatch is `REPLAN REQUIRED`, not a release-prep implementation task. The currency gate must derive its public-doc set without implementation-code, comment, or docstring references to planning-artifact paths; a required correction first needs a reviewed docs-only reconciliation prerequisite. Canonical metadata and current public version declarations must change in the same independently green PR because the full quality suite asserts current-documentation currency. |
