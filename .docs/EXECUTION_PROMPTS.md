...
1. PR-1 `build(release): prepare v0.5.0 metadata and dependency manifest` — scope: the justified release-tool currency-selection correction, its regression coverage, canonical version, changelog, dependency manifest, artifact metadata; commits: `fix(release): select public documentation for currency checks`, `build(release): prepare v0.5.0 metadata`, `docs(changelog): add v0.5.0 release entry`; verification: selection regression, lockfile, build, metadata, and hash checks. The release gate is intentionally deferred: after canonical metadata changes, its docs-currency precheck must fail until PR-2 updates public version declarations.
2. PR-2 `docs: sync v0.5.0 operator documentation` — scope: README and operator docs, label/config migration, evidence-PR operation, compatibility/known-limit statement; commits: `docs: sync measurable G4 operation for v0.5.0`; verification: docs-currency check, full release gate, and command examples against the installed wheel.
