...
1. PR-1 `fix(release): select public documentation for currency checks` — scope: only the justified release-tool currency-selection correction and mutation-verified regression coverage; commit: `fix(release): select public documentation for currency checks`; verification: focused regression, mutation guard, and full quality gate against the published `0.4.0` metadata and docs.
2. PR-2 `docs: sync v0.5.0 operator documentation` — scope: canonical version, changelog, dependency manifest, artifact metadata, README and operator docs, label/config migration, evidence-PR operation, compatibility/known-limit statement; commits: `build(release): prepare v0.5.0 metadata`, `docs(changelog): add v0.5.0 release entry`, `docs: sync measurable G4 operation for v0.5.0`; verification: docs-currency check, full release gate, build, metadata, hashes, and command examples against the installed wheel.
