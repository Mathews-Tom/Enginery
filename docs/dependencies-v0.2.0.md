...
`v0.2.0` release; full pinned versions and per-platform `sha256` wheel/sdist
...
`v0.2.0` adds no new runtime dependency over `v0.1.0`: the capability
locking, provenance verification, Claude Code adapter, plan ingestion,
and Stage 2 release-publication code added by this release train all
build on the same `cryptography`/optional `pyyaml` footprint already
shipped in `v0.1.0`.
...
clean `v0.2.0` install (`uv pip install enginery-0.2.0-py3-none-any.whl[armory]`)
in an isolated virtual environment:

| Name         | Version | License                    | URL                                                 |
|--------------|---------|----------------------------|------------------------------------------------------|
| enginery     | 0.2.0   | Apache-2.0                 | https://github.com/Mathews-Tom/Enginery              |
